...
Link your RStudio to your GitHub account and create a **public** repository where you will commit and push this assignment to. We will be following the instructions in [https://github.com/evanhorwitz/Horwitz_Evan_201_A06](https://happygitwithr.com/) The chapters listed below are referring to this book.
...
[https://github.com/evanhorwitz/Horwitz_Evan_201_A06](https://github.com/evanhorwitz/Horwitz_Evan_201_A06)
...
for(name in names(player_info)){
  # assign points / games_played to each name in list
...
for(i in points_per_game){
  points_per_game[[index]] <- round(i, 2)
  index <- index + 1
}
...
print(mat_rand)
# we define a function within the apply to round different amounts for each value
mat_rand <- apply(mat_rand, 1:2, function(x){
  # use nchar to check how long each numeric is
  # convert to char to use nchar
...
for(name in names(max_temps)){
  if(name != 'state' && name != 'year'){
    temp_avgs[[name]] <- mean((unlist(max_temps[[name]])))
  }
}
...
```{r}
col_temps <- filter(max_temps, state == 'CO')
oh_temps <- filter(max_temps, state == 'OH')
vir_temps <- filter(max_temps, state == 'VA')
...
```{r}
# create a new vector of medians using apply
col_medians <- apply(col_temps[, 3:14], 1, median)

# use which.max() as an index
paste0("The maximum median annual temperature of ", max(col_medians), " occured in ", col_temps[[2]][[which.max(col_medians)]], " and the minimum median annual temperature of ", min(col_medians), " occured in ", col_temps[[2]][[which.min(col_medians)]], "." )
...
sum <- function(n) {
  # define base case (if n is below 10 it is itself)
  if (n < 10) {return(n)} else {
    # use recursive to iterate through digits
    # %% by 10 yields most recent digit 
